...
flair = co.classify(
  ...
  inputs=[user_input],
  examples=[
    Example("Dr Green was very kind and helpful when I asked about gender affirming care options.", "LGBTQ friendly"),
    Example(
...
raw_print_statement_pos_or_neg = ('The confidence levels of the labels are: {}'.format(pos_or_neg.classifications)).split(",")
prediction_pos_or_neg = raw_print_statement_pos_or_neg[0].split(": ")[-1]
prediction_pos_or_neg = prediction_pos_or_neg[1:-1].lower()
print(f"This was a {prediction_pos_or_neg.upper()} experience")
...
for i in range(2, len(raw_print_statement_flair)):
  confidence_numbers.append((float(raw_print_statement_flair[i].split("=")[-1].split(")")[0])*100))
confidence_numbers_sorted = confidence_numbers[:]
confidence_numbers_sorted.sort()
second_highest_confidence_index = confidence_numbers.index(confidence_numbers_sorted[-2]) + 2


prediction_flair_second_highest = raw_print_statement_flair[second_highest_confidence_index].split(":")[0][2:-1].lower()



print(f"Flair: {prediction_flair.title()}")

if round(float(confidence_numbers_sorted[-2])) > 30:
    print(f"Second highest flair: {prediction_flair_second_highest.title()}")
